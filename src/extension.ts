...
import { NumpydocDocstring } from "./generate_docstring";
...
export function activate(context: vs.ExtensionContext): void {
    ...
    context.subscriptions.push(
        vs.commands.registerCommand(generateDocstringCommand, () => {
            ...
            const npDocstring = new NumpydocDocstring(editor);

            try {
                return npDocstring.generateDocstring();
            } catch (error) {
                ...
                language,
                {
                    provideCompletionItems: (document: vs.TextDocument, position: vs.Position) => {
                        if (validEnterActivation(document, position)) {
                            return [new NumpydocDocstringCompletionItem(document, position)];
                        }
                    },
                },
                '"',
    ...
    logInfo("NumPy-doc was activated");
}
 ...
 */
class NumpydocDocstringCompletionItem extends vs.CompletionItem {
