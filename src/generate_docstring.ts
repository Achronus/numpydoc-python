...
import { getCustomTemplate, getDefaultTemplate } from "./docstring/get_template";
...
export class NumpydocDocstring {
    ...
    private getTemplate(): string {
        ...
        if (customTemplatePath !== "") {
            if (!path.isAbsolute(customTemplatePath)) {
                ...
                if (workspaceFolder) {
                    customTemplatePath = path.join(workspaceFolder, customTemplatePath);
                }
            }
            return getCustomTemplate(customTemplatePath);
        }

        // Otherwise use default numpy template
